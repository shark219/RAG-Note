...
let vertexCounter = 0;
let config = mermaid_core.getConfig();
let vertices = {};
let edges = [];
let classes = {};
let subGraphs = [];
...
let version;
let funs = [];
const sanitizeText = (txt) => mermaid_core.common.sanitizeText(txt, config);
...
const updateLink = function(positions, style) {
  positions.forEach(function(pos) {
    if (pos === "default") {
      ...
      if (mermaid_core.utils.isSubstringInArray("fill", style) === -1) {
...
const setLink = function(ids, linkStr, target) {
  ids.split(",").forEach(function(id) {
    if (vertices[id] !== void 0) {
      vertices[id].link = mermaid_core.utils.formatUrl(linkStr, config);
...
const setupToolTips = function(element) {
  let tooltipElem = mermaid_core.select(".mermaidTooltip");
  if ((tooltipElem._groups || tooltipElem)[0][0] === null) {
    tooltipElem = mermaid_core.select("body").append("div").attr("class", "mermaidTooltip").style("opacity", 0);
  }
  const svg = mermaid_core.select(element).select("svg");
  const nodes = svg.selectAll("g.node");
  nodes.on("mouseover", function() {
    const el = mermaid_core.select(this);
...
const addSubGraph = function(_id, list, _title) {
  ...
  mermaid_core.log.info("Adding", subGraph.id, subGraph.nodes, subGraph.dir);
...
const flowDb = {
  parseDirective,
  defaultConfig: () => mermaid_core.defaultConfig.flowchart,
...
function applyTransition(selection, g) {
  var graph = g.graph();
  if (isPlainObject.isPlainObject(graph)) {
    var transition = graph.transition;
    if (mermaid_core.isFunction(transition)) {
