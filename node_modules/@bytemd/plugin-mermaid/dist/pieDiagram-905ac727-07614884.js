...
const arc = require("./arc-e9426649.js");
...
function pie() {
  var value = identity, sortValues = descending, sort = null, startAngle = path.constant(0), endAngle = path.constant(mermaid_core.tau), padAngle = path.constant(0);
  function pie2(data) {
    var i, n = (data = array.array(data)).length, j, k, sum = 0, index = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(mermaid_core.tau, Math.max(-mermaid_core.tau, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
    for (i = 0; i < n; ++i) {
  ...
  pie2.value = function(_) {
    return arguments.length ? (value = typeof _ === "function" ? _ : path.constant(+_), pie2) : value;
  };
  ...
  pie2.startAngle = function(_) {
    return arguments.length ? (startAngle = typeof _ === "function" ? _ : path.constant(+_), pie2) : startAngle;
  };
  pie2.endAngle = function(_) {
    return arguments.length ? (endAngle = typeof _ === "function" ? _ : path.constant(+_), pie2) : endAngle;
  };
  pie2.padAngle = function(_) {
    return arguments.length ? (padAngle = typeof _ === "function" ? _ : path.constant(+_), pie2) : padAngle;
  };
  return pie2;
}
var parser = function() {
...
const addSection = function(id, value) {
  ...
  if (sections[id] === void 0) {
    sections[id] = value;
    mermaid_core.log.debug("Added new section :", id);
  }
};
...
const db = {
  parseDirective,
  getConfig: () => mermaid_core.getConfig().pie,
  addSection,
  getSections,
  cleanupValue,
  clear,
...
let conf = mermaid_core.getConfig();
let width;
const height = 450;
const draw = (txt, id, _version, diagObj) => {
  try {
    conf = mermaid_core.getConfig();
    mermaid_core.log.debug("Rendering info diagram\n" + txt);
    ...
    mermaid_core.log.debug("Parsed info diagram");
    ...
    mermaid_core.configureSvgSize(diagram2, height, width, conf.pie.useMaxWidth);
    ...
    var arcGenerator = arc.arc().innerRadius(0).outerRadius(radius);
    ...
    mermaid_core.log.error("Error while rendering info diagram");
