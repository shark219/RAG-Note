...
let mainBranchName = mermaid_core.getConfig().gitGraph.mainBranchName;
let mainBranchOrder = mermaid_core.getConfig().gitGraph.mainBranchOrder;
let commits = {};
let head = null;
let branchesConfig = {};
...
let seq = 0;
function getId() {
  return mermaid_core.random({ length: 7 });
}
...
const setOptions = function(rawOptString) {
  mermaid_core.log.debug("options str", rawOptString);
  rawOptString = rawOptString && rawOptString.trim();
  rawOptString = rawOptString || "{}";
  try {
    options = JSON.parse(rawOptString);
  } catch (e) {
    mermaid_core.log.error("error while parsing gitGraph options", e.message);
  }
};
...
const commit = function(msg, id, type, tag) {
  mermaid_core.log.debug("Entering commit:", msg, id, type, tag);
  id = mermaid_core.common.sanitizeText(id, mermaid_core.getConfig());
  msg = mermaid_core.common.sanitizeText(msg, mermaid_core.getConfig());
  tag = mermaid_core.common.sanitizeText(tag, mermaid_core.getConfig());
  const commit2 = {
    id: id ? id : seq + "-" + getId(),
    message: msg,
    seq: seq++,
  ...
  mermaid_core.log.debug("in pushCommit " + commit2.id);
};
const branch = function(name, order) {
  name = mermaid_core.common.sanitizeText(name, mermaid_core.getConfig());
  if (branches[name] === void 0) {
    branches[name] = head != null ? head.id : null;
    branchesConfig[name] = { name, order: order ? parseInt(order, 10) : null };
    checkout(name);
    mermaid_core.log.debug("in createBranch");
  } else {
    let error = new Error(
...
const merge = function(otherBranch, custom_id, override_type, custom_tag) {
  otherBranch = mermaid_core.common.sanitizeText(otherBranch, mermaid_core.getConfig());
  custom_id = mermaid_core.common.sanitizeText(custom_id, mermaid_core.getConfig());
  ...
  commits[commit2.id] = commit2;
  branches[curBranch] = commit2.id;
  mermaid_core.log.debug(branches);
  mermaid_core.log.debug("in mergeBranch");
};
const cherryPick = function(sourceId, targetId, tag) {
  mermaid_core.log.debug("Entering cherryPick:", sourceId, targetId, tag);
  sourceId = mermaid_core.common.sanitizeText(sourceId, mermaid_core.getConfig());
  targetId = mermaid_core.common.sanitizeText(targetId, mermaid_core.getConfig());
  tag = mermaid_core.common.sanitizeText(tag, mermaid_core.getConfig());
  ...
  if (!targetId || commits[targetId] === void 0) {
    ...
    branches[curBranch] = commit2.id;
    mermaid_core.log.debug(branches);
    mermaid_core.log.debug("in cherryPick");
  }
};
const checkout = function(branch2) {
  branch2 = mermaid_core.common.sanitizeText(branch2, mermaid_core.getConfig());
...
function prettyPrintCommitHistory(commitArr) {
  ...
  mermaid_core.log.debug(label.join(" "));
...
const prettyPrint = function() {
  mermaid_core.log.debug(commits);
...
const clear$1 = function() {
  commits = {};
  head = null;
  let mainBranch = mermaid_core.getConfig().gitGraph.mainBranchName;
  let mainBranchOrder2 = mermaid_core.getConfig().gitGraph.mainBranchOrder;
  branches = {};
...
const gitGraphDb = {
  parseDirective,
  getConfig: () => mermaid_core.getConfig().gitGraph,
  setDirection,
  setOptions,
  getOptions,
  commit,
...
const drawCommits = (svg, commits2, modifyGraph) => {
  const gitGraphConfig = mermaid_core.getConfig$1().gitGraph;
...
const draw = function(txt, id, ver, diagObj) {
  var _a;
  clear();
  const conf = mermaid_core.getConfig$1();
  const gitGraphConfig = conf.gitGraph;
  mermaid_core.log.debug("in gitgraph renderer", txt + "\n", "id:", id, ver);
  ...
  const diagram2 = mermaid_core.select(`[id="${id}"]`);
  ...
  mermaid_core.utils.insertTitle(
    diagram2,
    "gitTitleText",
    gitGraphConfig.titleTopMargin,
    diagObj.db.getDiagramTitle()
  );
  mermaid_core.setupGraphViewbox$1(
    void 0,
    diagram2,
    gitGraphConfig.diagramPadding,
    (_a = gitGraphConfig.useMaxWidth) != null ? _a : conf.useMaxWidth
  );
};
