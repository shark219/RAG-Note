...
const parseDirective = function(statement, context, type) {
  mermaid_core.mermaidAPI.parseDirective(this, statement, context, type);
};
...
const autoWrap = () => {
  ...
  return mermaid_core.getConfig().sequence.wrap;
};
...
const parseMessage = function(str) {
  ...
  mermaid_core.log.debug("parseMessage:", message);
  return message;
};
const parseBoxData = function(str) {
  ...
  const boxData = {
    color,
    text: title !== void 0 ? mermaid_core.sanitizeText(title.replace(/^:?(?:no)?wrap:/, ""), mermaid_core.getConfig()) : void 0,
...
const addLinks = function(actorId, text) {
  ...
  try {
    let sanitizedText = mermaid_core.sanitizeText(text.text, mermaid_core.getConfig());
    sanitizedText = sanitizedText.replace(/&amp;/g, "&");
    sanitizedText = sanitizedText.replace(/&equals;/g, "=");
    const links = JSON.parse(sanitizedText);
    insertLinks(actor, links);
  } catch (e) {
    mermaid_core.log.error("error while parsing actor link text", e);
  }
};
...
const addProperties = function(actorId, text) {
  ...
  try {
    ...
    mermaid_core.log.error("error while parsing actor properties text", e);
  }
};
...
const addDetails = function(actorId, text) {
  ...
  try {
    ...
    mermaid_core.log.error("error while parsing actor details text", e);
  }
};
  ...
  if (Array.isArray(param)) {
    ...
    switch (param.type) {
      ...
      case "altEnd":
        ...
        break;
      case "setAccTitle":
        mermaid_core.setAccTitle(param.text);
        break;
      case "parStart":
...
const db = {
  ...
  getActorKeys,
  getActorProperty,
  getAccTitle: mermaid_core.getAccTitle,
  getBoxes,
  getDiagramTitle: mermaid_core.getDiagramTitle,
  setDiagramTitle: mermaid_core.setDiagramTitle,
  parseDirective,
  getConfig: () => mermaid_core.getConfig().sequence,
  clear,
  parseMessage,
  parseBoxData,
  LINETYPE,
  ...
  PLACEMENT,
  addNote,
  setAccTitle: mermaid_core.setAccTitle,
  apply,
  setAccDescription: mermaid_core.setAccDescription,
  getAccDescription: mermaid_core.getAccDescription,
  hasAtLeastOneBox,
...
const drawPopup = function(elem, actor, minMenuWidth, textAttrs, forceMenus) {
  ...
  if (links != null) {
    ...
    for (let key in links) {
      ...
      var sanitizedLink = mermaid_core.sanitizeUrl_1(links[key]);
...
const drawImage = function(elem, x, y, link) {
  ...
  imageElem.attr("x", x);
  imageElem.attr("y", y);
  var sanitizedLink = mermaid_core.sanitizeUrl_1(link);
...
const drawText = function(elem, textData) {
  ...
  const lines = textData.text.split(mermaid_core.common.lineBreakRegex);
  const [_textFontSize, _textFontSizePx] = mermaid_core.parseFontSize(textData.fontSize);
  let textElems = [];
  let dy = 0;
...
const _drawTextCandidateFunc = function() {
  ...
  function byTspan(content, g, x, y, width, height, textAttrs, conf2) {
    ...
    const [_actorFontSize, _actorFontSizePx] = mermaid_core.parseFontSize(actorFontSize);
    const lines = content.split(mermaid_core.common.lineBreakRegex);
...
const svgDraw = {
  ...
  popdownMenu,
  fixLifeLineHeights,
  sanitizeUrl: mermaid_core.sanitizeUrl_1
};
let conf = {};
const bounds = {
  data: {
  ...
  init: function() {
    ...
    this.verticalPos = 0;
    setConf(mermaid_core.getConfig());
  },
...
function boundMessage(_diagram, msgModel) {
  ...
  const lines = mermaid_core.common.splitBreaks(message).length;
  const textDims = mermaid_core.utils.calculateTextDimensions(message, messageFont(conf));
...
const setConf = function(cnf) {
  mermaid_core.assignWithDepth$1(conf, cnf);
  if (cnf.fontFamily) {
...
function adjustLoopHeightForWrap(loopWidths, msg, preMargin, postMargin, addLoopFn) {
  ...
  if (msg.id && msg.message && loopWidths[msg.id]) {
    ...
    msg.message = mermaid_core.utils.wrapLabel(`[${msg.message}]`, loopWidth - 2 * conf.wrapPadding, textConf);
    msg.width = loopWidth;
    msg.wrap = true;
    const textDims = mermaid_core.utils.calculateTextDimensions(msg.message, textConf);
    const totalOffset = Math.max(textDims.height, conf.labelBoxHeight);
    heightAdjust = postMargin + totalOffset;
    mermaid_core.log.debug(`${totalOffset} - ${msg.message}`);
  }
  addLoopFn(msg);
  bounds.bumpVerticalPos(heightAdjust);
}
const draw = function(_text, id, _version, diagObj) {
  const { securityLevel, sequence } = mermaid_core.getConfig();
  conf = sequence;
  diagObj.db.clear();
  ...
  bounds.init();
  mermaid_core.log.debug(diagObj.db);
  const diagram2 = securityLevel === "sandbox" ? root.select(`[id="${id}"]`) : mermaid_core.select(`[id="${id}"]`);
    ...
    switch (msg.type) {
      ...
      default:
        try {
          ...
          mermaid_core.log.error("error while drawing message", e);
        }
    }
    if ([
  ...
  mermaid_core.log.debug("For line height fix Querying: #" + id + " .actor-line");
  const actorLines = selectAll.selectAll("#" + id + " .actor-line");
  ...
  mermaid_core.log.debug(`models:`, bounds.models);
};
function getMaxMessageWidthPerActor(actors2, messages2, diagObj) {
  ...
  messages2.forEach(function(msg) {
    if (actors2[msg.to] && actors2[msg.from]) {
      ...
      const wrappedMessage = msg.wrap ? mermaid_core.utils.wrapLabel(msg.message, conf.width - 2 * conf.wrapPadding, textFont) : msg.message;
      const messageDimensions = mermaid_core.utils.calculateTextDimensions(wrappedMessage, textFont);
  ...
  mermaid_core.log.debug("maxMessageWidthPerActor:", maxMessageWidthPerActor);
...
const getRequiredPopupWidth = function(actor) {
  ...
  for (const key in actor.links) {
    const labelDimensions = mermaid_core.utils.calculateTextDimensions(key, textFont);
...
function calculateActorMargins(actors2, actorToMessageWidth, boxes2) {
  ...
  Object.keys(actors2).forEach((prop) => {
    ...
    if (actor.wrap) {
      actor.description = mermaid_core.utils.wrapLabel(
        actor.description,
        conf.width - 2 * conf.wrapPadding,
        actorFont(conf)
      );
    }
    const actDims = mermaid_core.utils.calculateTextDimensions(actor.description, actorFont(conf));
  ...
  boxes2.forEach((box) => {
    ...
    if (box.wrap) {
      box.name = mermaid_core.utils.wrapLabel(box.name, totalWidth - 2 * conf.wrapPadding, textFont);
    }
    const boxMsgDimensions = mermaid_core.utils.calculateTextDimensions(box.name, textFont);
...
const buildNoteModel = function(msg, actors2, diagObj) {
  ...
  let textDimensions = mermaid_core.utils.calculateTextDimensions(
    shouldWrap ? mermaid_core.utils.wrapLabel(msg.message, conf.width, noteFont(conf)) : msg.message,
    noteFont(conf)
  );
  const noteModel = {
  ...
  if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF) {
    ...
    textDimensions = mermaid_core.utils.calculateTextDimensions(
      shouldWrap ? mermaid_core.utils.wrapLabel(msg.message, Math.max(conf.width, actors2[msg.from].width), noteFont(conf)) : msg.message,
      noteFont(conf)
    );
  ...
  if (shouldWrap) {
    noteModel.message = mermaid_core.utils.wrapLabel(
      msg.message,
...
const buildMessageModel = function(msg, actors2, diagObj) {
  ...
  if (msg.wrap && msg.message) {
    msg.message = mermaid_core.utils.wrapLabel(
      msg.message,
      Math.max(boundedWidth + 2 * conf.wrapPadding, conf.width),
      messageFont(conf)
    );
  }
  const msgDims = mermaid_core.utils.calculateTextDimensions(msg.message, messageFont(conf));
  return {
    width: Math.max(
...
const calculateLoopBounds = function(messages2, actors2, _maxWidthPerActor, diagObj) {
  ...
  messages2.forEach(function(msg) {
    msg.id = mermaid_core.utils.random({ length: 10 });
    switch (msg.type) {
      case diagObj.db.LINETYPE.LOOP_START:
      case diagObj.db.LINETYPE.ALT_START:
      case diagObj.db.LINETYPE.OPT_START:
  ...
  bounds.activations = [];
  mermaid_core.log.debug("Loop type widths:", loops);
  return loops;
};
const renderer = {
  bounds,
