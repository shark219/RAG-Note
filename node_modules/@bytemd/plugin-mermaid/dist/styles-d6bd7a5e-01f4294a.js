...
const setRootDoc = (o) => {
  mermaid_core.log.info("Setting root doc", o);
  rootDoc = o;
};
...
const extract = (_doc) => {
  ...
  mermaid_core.log.info(doc);
  clear(true);
  mermaid_core.log.info("Extract", doc);
  doc.forEach((item) => {
    switch (item.stmt) {
      case STMT_STATE:
        addState(
...
const addState = function(id, type = DEFAULT_STATE_TYPE, doc = null, descr = null, note = null, classes2 = null, styles2 = null, textStyles = null) {
  ...
  if (currentDocument.states[trimmedId] === void 0) {
    mermaid_core.log.info("Adding state ", trimmedId, descr);
  ...
  if (descr) {
    mermaid_core.log.info("Setting state description", trimmedId, descr);
  ...
  if (note) {
    ...
    currentDocument.states[trimmedId].note.text = mermaid_core.common.sanitizeText(
      currentDocument.states[trimmedId].note.text,
      mermaid_core.getConfig()
    );
  }
  if (classes2) {
    mermaid_core.log.info("Setting state classes", trimmedId, classes2);
    const classesList = typeof classes2 === "string" ? [classes2] : classes2;
    classesList.forEach((klass) => setCssClass(trimmedId, klass.trim()));
  }
  if (styles2) {
    mermaid_core.log.info("Setting state styles", trimmedId, styles2);
...
const logDocuments = function() {
  mermaid_core.log.info("Documents = ", documents);
};
...
function addRelationObjs(item1, item2, relationTitle) {
  ...
  currentDocument.relations.push({
    id1,
    id2,
    relationTitle: mermaid_core.common.sanitizeText(relationTitle, mermaid_core.getConfig())
  });
}
const addRelation = function(item1, item2, title) {
  if (typeof item1 === "object") {
    ...
    currentDocument.relations.push({
      id1,
      id2,
      title: mermaid_core.common.sanitizeText(title, mermaid_core.getConfig())
    });
  }
};
const addDescription = function(id, descr) {
  ...
  theState.descriptions.push(mermaid_core.common.sanitizeText(_descr, mermaid_core.getConfig()));
};
...
const db = {
  parseDirective,
  getConfig: () => mermaid_core.getConfig().state,
  addState,
  clear,
  getState,
  getStates,
