...
const setC4Type = function(c4TypeParam) {
  let sanitizedText = mermaid_core.sanitizeText(c4TypeParam, mermaid_core.getConfig());
...
const setTitle = function(txt) {
  let sanitizedText = mermaid_core.sanitizeText(txt, mermaid_core.getConfig());
  title = sanitizedText;
};
const c4Db = {
  addPersonOrSystem,
  ...
  parseDirective,
  getConfig: () => mermaid_core.getConfig().c4,
  clear,
  LINETYPE,
  ARROWTYPE,
  PLACEMENT,
...
const drawImage = function(elem, width, height, x, y, link) {
  ...
  imageElem.attr("x", x);
  imageElem.attr("y", y);
  let sanitizedLink = link.startsWith("data:image/png;base64") ? link : mermaid_core.sanitizeUrl_1(link);
...
function calcC4ShapeTextWH(textType, c4Shape, c4ShapeTextWrap, textConf, textLimitWidth) {
  if (!c4Shape[textType].width) {
    if (c4ShapeTextWrap) {
      c4Shape[textType].text = mermaid_core.wrapLabel(c4Shape[textType].text, textLimitWidth, textConf);
      c4Shape[textType].textLines = c4Shape[textType].text.split(mermaid_core.common.lineBreakRegex).length;
      c4Shape[textType].width = textLimitWidth;
      c4Shape[textType].height = mermaid_core.calculateTextHeight(c4Shape[textType].text, textConf);
    } else {
      let lines = c4Shape[textType].text.split(mermaid_core.common.lineBreakRegex);
      ...
      for (const line of lines) {
        c4Shape[textType].width = Math.max(
          mermaid_core.calculateTextWidth(line, textConf),
          c4Shape[textType].width
        );
        lineHeight = mermaid_core.calculateTextHeight(line, textConf);
...
const drawBoundary = function(diagram2, boundary, bounds) {
  ...
  let textLimitWidth = mermaid_core.calculateTextWidth(boundary.label.text, boundaryLabelConf);
...
const drawC4ShapeArray = function(currentBounds, diagram2, c4ShapeArray2, c4ShapeKeys) {
  ...
  for (const c4ShapeKey of c4ShapeKeys) {
    ...
    c4Shape.typeC4Shape.width = mermaid_core.calculateTextWidth(
...
const drawRels = function(diagram2, rels2, getC4ShapeObj, diagObj) {
  ...
  for (let rel of rels2) {
    ...
    let textLimitWidth = mermaid_core.calculateTextWidth(rel.label.text, relConf);
    calcC4ShapeTextWH("label", rel, relTextWrap, relConf, textLimitWidth);
    if (rel.techn && rel.techn.text !== "") {
      textLimitWidth = mermaid_core.calculateTextWidth(rel.techn.text, relConf);
      calcC4ShapeTextWH("techn", rel, relTextWrap, relConf, textLimitWidth);
    }
    if (rel.descr && rel.descr.text !== "") {
      textLimitWidth = mermaid_core.calculateTextWidth(rel.descr.text, relConf);
...
const draw = function(_text, id, _version, diagObj) {
  conf = mermaid_core.getConfig().c4;
  ...
  mermaid_core.log.debug(`C:${JSON.stringify(conf, null, 2)}`);
  ...
  mermaid_core.log.debug(`models:`, box);
};
const c4Renderer = {
