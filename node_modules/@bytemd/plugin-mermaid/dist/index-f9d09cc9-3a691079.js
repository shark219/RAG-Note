...
const edges65da65dc = require("./edges-65da65dc-d0d46691.js");
...
const rect = (parent, node) => {
  mermaid_core.log.trace("Creating subgraph rect for ", node.id, node);
  const shapeSvg = parent.insert("g").attr("class", "cluster" + (node.class ? " " + node.class : "")).attr("id", node.id);
  const rect2 = shapeSvg.insert("rect", ":first-child");
  const label = shapeSvg.insert("g").attr("class", "cluster-label");
  const text = label.node().appendChild(edges65da65dc.createLabel$1(node.labelText, node.labelStyle, void 0, true));
  ...
  node.intersect = function(point) {
    return edges65da65dc.intersectRect$1(node, point);
  };
  return shapeSvg;
};
...
const recursiveRender = (_elem, graph, diagramtype, parentCluster) => {
  ...
  graph.nodes().forEach(function(v) {
    ...
    if (node && node.clusterNode) {
      ...
      const newEl = o.elem;
      edges65da65dc.updateNodeBounds(node, newEl);
      node.diff = o.diff || 0;
      mermaid_core.log.info("Node bounds (abc123)", v, node, node.width, node.x, node.y);
      edges65da65dc.setNodeElem(newEl, node);
      ...
      if (graph.children(v).length > 0) {
        ...
        edges65da65dc.insertNode(nodes, graph.node(v), dir);
      }
    }
  });
  graph.edges().forEach(function(e) {
    ...
    edges65da65dc.insertEdgeLabel(edgeLabels, edge);
  });
  graph.edges().forEach(function(e) {
    mermaid_core.log.info("Edge " + e.v + " -> " + e.w + ": " + JSON.stringify(e));
  });
  ...
  sortNodesByHierarchy(graph).forEach(function(v) {
    ...
    if (node && node.clusterNode) {
      edges65da65dc.positionNode(node);
    } else {
      if (graph.children(v).length > 0) {
  ...
  graph.edges().forEach(function(e) {
    ...
    const paths = edges65da65dc.insertEdge(edgePaths, e, edge, clusterDb, diagramtype, graph);
    edges65da65dc.positionEdgeLabel(edge, paths);
  });
  graph.nodes().forEach(function(v) {
...
const render = (elem, graph, markers, diagramtype, id) => {
  edges65da65dc.insertMarkers$1(elem, markers, diagramtype, id);
  edges65da65dc.clear$1();
  edges65da65dc.clear();
  clear();
  clear$1();
