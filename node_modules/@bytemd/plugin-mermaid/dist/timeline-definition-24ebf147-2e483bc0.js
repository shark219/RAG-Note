...
const is_dark = require("./is_dark-415b19c3.js");
...
const tasks = [];
const rawTasks = [];
const getCommonDb = () => mermaid_core.commonDb$1;
const parseDirective = (statement, context, type) => {
  mermaid_core.parseDirective$1(globalThis, statement, context, type);
};
...
function wrap(text, width) {
  text.each(function() {
    var text2 = mermaid_core.select(this), words = text2.text().split(/(\s+|<br>)/).reverse(), word, line = [], lineHeight = 1.1, y = text2.attr("y"), dy = parseFloat(text2.attr("dy")), tspan = text2.text(null).append("tspan").attr("x", 0).attr("y", y).attr("dy", dy + "em");
...
const draw = function(text, id, version, diagObj) {
  var _a2, _b2, _c2;
  var _a, _b, _c, _d;
  const conf = mermaid_core.getConfig();
  const LEFT_MARGIN = (_a2 = conf.leftMargin) != null ? _a2 : 50;
  (_b = (_a = diagObj.db).clear) == null ? void 0 : _b.call(_a);
  diagObj.parser.parse(text + "\n");
  mermaid_core.log.debug("timeline", diagObj.db);
  ...
  mermaid_core.log.debug("task", tasks2);
  svgDraw.initGraphics(svg);
  const sections2 = diagObj.db.getSections();
  mermaid_core.log.debug("sections", sections2);
  ...
  sections2.forEach(function(section) {
    ...
    mermaid_core.log.debug("sectionHeight before draw", sectionHeight);
    maxSectionHeight = Math.max(maxSectionHeight, sectionHeight + 20);
  });
  let maxEventCount = 0;
  let maxEventLineLength = 0;
  mermaid_core.log.debug("tasks.length", tasks2.length);
  for (const [i, task] of tasks2.entries()) {
    ...
    mermaid_core.log.debug("taskHeight before draw", taskHeight);
  ...
  mermaid_core.log.debug("maxSectionHeight before draw", maxSectionHeight);
  mermaid_core.log.debug("maxTaskHeight before draw", maxTaskHeight);
  if (sections2 && sections2.length > 0) {
    sections2.forEach((section) => {
      ...
      mermaid_core.log.debug("sectionNode", sectionNode);
      const sectionNodeWrapper = svg.append("g");
      const node = svgDraw.drawNode(sectionNodeWrapper, sectionNode, sectionNumber, conf);
      mermaid_core.log.debug("sectionNode output", node);
  ...
  mermaid_core.log.debug("bounds", box);
  if (title) {
  ...
  mermaid_core.setupGraphViewbox(
    void 0,
    svg,
    (_b2 = (_c = conf.timeline) == null ? void 0 : _c.padding) != null ? _b2 : 50,
    (_c2 = (_d = conf.timeline) == null ? void 0 : _d.useMaxWidth) != null ? _c2 : false
  );
};
const drawTasks = function(diagram2, tasks2, sectionColor, masterX, masterY, maxTaskHeight, conf, maxEventCount, maxEventLineLength, maxSectionHeight, isWithoutSections) {
  ...
  for (const task of tasks2) {
    ...
    mermaid_core.log.debug("taskNode", taskNode);
    const taskWrapper = diagram2.append("g").attr("class", "taskWrapper");
    const node = svgDraw.drawNode(taskWrapper, taskNode, sectionColor, conf);
    const taskHeight = node.height;
    mermaid_core.log.debug("taskHeight after draw", taskHeight);
...
const drawEvents = function(diagram2, events, sectionColor, masterX, masterY, conf) {
  ...
  for (const event of events) {
    ...
    mermaid_core.log.debug("eventNode", eventNode);
...
const genSections = (options) => {
  ...
  for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
    ...
    if (is_dark.isDark(options["lineColor" + i])) {
      options["lineColor" + i] = mermaid_core.lighten(options["lineColor" + i], 20);
    } else {
      options["lineColor" + i] = mermaid_core.darken(options["lineColor" + i], 20);
    }
  }
  for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
    const sw = "" + (17 - 3 * i);
