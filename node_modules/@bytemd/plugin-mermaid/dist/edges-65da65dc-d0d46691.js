...
const svgDraw6a237a99 = require("./svgDraw-6a237a99-9b78f7fb.js");
...
const extension = (elem, type, id) => {
  mermaid_core.log.trace("Making markers for ", id);
...
function addHtmlLabel(node) {
  const fo = mermaid_core.select(document.createElementNS("http://www.w3.org/2000/svg", "foreignObject"));
...
const createLabel = (_vertexText, style, isTitle, isNode) => {
  ...
  if (mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels)) {
    ...
    mermaid_core.log.info("vertexText" + vertexText);
    const node = {
      isNode,
      label: mermaid_core.decodeEntities(vertexText).replace(
        /fa[blrs]?:fa-[\w-]+/g,
...
const labelHelper = (parent, node, _classes, isNode) => {
  ...
  const text = label.node().appendChild(
    createLabel$1(
      mermaid_core.sanitizeText(mermaid_core.decodeEntities(labelText), mermaid_core.getConfig()),
      node.labelStyle,
      false,
      isNode
    )
...
const note = (parent, node) => {
  ...
  mermaid_core.log.info("Classes = ", node.classes);
...
const question = (parent, node) => {
  ...
  mermaid_core.log.info("Question main (Circle)");
  const questionElem = insertPolygonShape(shapeSvg, s, s, points);
  questionElem.attr("style", node.style);
  updateNodeBounds(node, questionElem);
  node.intersect = function(point2) {
    mermaid_core.log.warn("Intersect called");
...
const rect = (parent, node) => {
  const { shapeSvg, bbox, halfPadding } = labelHelper(parent, node, "node " + node.classes, true);
  mermaid_core.log.trace("Classes = ", node.classes);
  const rect2 = shapeSvg.insert("rect", ":first-child");
  ...
  if (node.props) {
    ...
    propKeys.forEach((propKey) => {
      mermaid_core.log.warn(`Unknown node property ${propKey}`);
    });
  }
  updateNodeBounds(node, rect2);
  node.intersect = function(point2) {
...
function applyNodePropertyBorders(rect2, borders, totalWidth, totalHeight) {
  ...
  if (borders.includes("t")) {
    mermaid_core.log.debug("add top border");
    addBorder(totalWidth);
  } else {
    skipBorder(totalWidth);
  }
  if (borders.includes("r")) {
    mermaid_core.log.debug("add right border");
    addBorder(totalHeight);
  } else {
    skipBorder(totalHeight);
  }
  if (borders.includes("b")) {
    mermaid_core.log.debug("add bottom border");
    addBorder(totalWidth);
  } else {
    skipBorder(totalWidth);
  }
  if (borders.includes("l")) {
    mermaid_core.log.debug("add left border");
    addBorder(totalHeight);
  } else {
    skipBorder(totalHeight);
  }
...
const rectWithTitle = (parent, node) => {
  ...
  mermaid_core.log.info("Label text abc79", title, text2, typeof text2 === "object");
  ...
  mermaid_core.log.info("Text 2", text2);
  ...
  if (mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels)) {
    ...
    const dv = mermaid_core.select(descr);
    bbox = div.getBoundingClientRect();
    dv.attr("width", bbox.width);
    dv.attr("height", bbox.height);
  }
  const halfPadding = node.padding / 2;
  mermaid_core.select(descr).attr(
    "transform",
    "translate( " + // (titleBox.width - bbox.width) / 2 +
    (bbox.width > titleBox.width ? 0 : (titleBox.width - bbox.width) / 2) + ", " + (titleBox.height + halfPadding + 5) + ")"
  );
  mermaid_core.select(text).attr(
    "transform",
    "translate( " + // (titleBox.width - bbox.width) / 2 +
...
const circle = (parent, node) => {
  ...
  mermaid_core.log.info("Circle main");
  updateNodeBounds(node, circle2);
  node.intersect = function(point2) {
    mermaid_core.log.info("Circle intersect", node, bbox.width / 2 + halfPadding, point2);
...
const doublecircle = (parent, node) => {
  ...
  mermaid_core.log.info("DoubleCircle main");
  updateNodeBounds(node, outerCircle);
  node.intersect = function(point2) {
    mermaid_core.log.info("DoubleCircle intersect", node, bbox.width / 2 + halfPadding + gap, point2);
...
const class_box = (parent, node) => {
  ...
  if (mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels)) {
    ...
    const dv = mermaid_core.select(interfaceLabel);
  ...
  if (node.classData.type !== void 0 && node.classData.type !== "") {
    if (mermaid_core.getConfig().flowchart.htmlLabels) {
  ...
  mermaid_core.select(classTitleLabel).attr("class", "classTitle");
  let classTitleBBox = classTitleLabel.getBBox();
  if (mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels)) {
    const div = classTitleLabel.children[0];
    const dv = mermaid_core.select(classTitleLabel);
  ...
  node.classData.members.forEach((str) => {
    const parsedInfo = svgDraw6a237a99.parseMember(str);
    ...
    if (mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels)) {
      ...
      const dv = mermaid_core.select(lbl);
      bbox = div.getBoundingClientRect();
      dv.attr("width", bbox.width);
      dv.attr("height", bbox.height);
    }
  ...
  if (hasInterface) {
    ...
    mermaid_core.select(interfaceLabel).attr(
      "transform",
  ...
  mermaid_core.select(classTitleLabel).attr(
    "transform",
...
const insertNode = (elem, node, dir) => {
  ...
  if (node.link) {
    let target;
    if (mermaid_core.getConfig().securityLevel === "sandbox") {
      target = "_top";
...
const insertEdgeLabel = (elem, edge) => {
  ...
  if (mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels)) {
    ...
    const dv = mermaid_core.select(labelElement);
    bbox = div.getBoundingClientRect();
    dv.attr("width", bbox.width);
    dv.attr("height", bbox.height);
  }
...
function setTerminalWidth(fo, value) {
  if (mermaid_core.getConfig().flowchart.htmlLabels && fo) {
    fo.style.width = value.length * 9 + "px";
    fo.style.height = "12px";
  }
}
const positionEdgeLabel = (edge, paths) => {
  mermaid_core.log.info("Moving label abc78 ", edge.id, edge.label, edgeLabels[edge.id]);
  ...
  if (edge.label) {
    ...
    let y = edge.y;
    if (path) {
      const pos = mermaid_core.utils.calcLabelPosition(path);
  ...
  if (edge.startLabelLeft) {
    ...
    let y = edge.y;
    if (path) {
      const pos = mermaid_core.utils.calcTerminalLabelPosition(edge.arrowTypeStart ? 10 : 0, "start_left", path);
      x = pos.x;
      y = pos.y;
    }
    el.attr("transform", "translate(" + x + ", " + y + ")");
  ...
  if (edge.startLabelRight) {
    ...
    let y = edge.y;
    if (path) {
      const pos = mermaid_core.utils.calcTerminalLabelPosition(
  ...
  if (edge.endLabelLeft) {
    ...
    let y = edge.y;
    if (path) {
      const pos = mermaid_core.utils.calcTerminalLabelPosition(edge.arrowTypeEnd ? 10 : 0, "end_left", path);
      x = pos.x;
      y = pos.y;
    }
    el.attr("transform", "translate(" + x + ", " + y + ")");
  ...
  if (edge.endLabelRight) {
    ...
    let y = edge.y;
    if (path) {
      const pos = mermaid_core.utils.calcTerminalLabelPosition(edge.arrowTypeEnd ? 10 : 0, "end_right", path);
      x = pos.x;
      y = pos.y;
    }
    el.attr("transform", "translate(" + x + ", " + y + ")");
...
const intersection = (node, outsidePoint, insidePoint) => {
  mermaid_core.log.warn(`intersection calc abc89:
  ...
  if (Math.abs(y - outsidePoint.y) * w > Math.abs(x - outsidePoint.x) * h) {
    ...
    mermaid_core.log.warn(`abc89 topp/bott calc, Q ${Q}, q ${q}, R ${R}, r ${r}`, res);
    return res;
  } else {
    ...
    mermaid_core.log.warn(`sides calc abc89, Q ${Q}, q ${q}, R ${R}, r ${r}`, { _x, _y });
    if (r === 0) {
      _x = outsidePoint.x;
      _y = outsidePoint.y;
    }
...
const cutPathAtIntersect = (_points, boundryNode) => {
  mermaid_core.log.warn("abc88 cutPathAtIntersect", _points, boundryNode);
  let points = [];
  let lastPointOutside = _points[0];
  let isInside = false;
  _points.forEach((point2) => {
    mermaid_core.log.info("abc88 checking point", point2, boundryNode);
    if (!outsideNode(boundryNode, point2) && !isInside) {
      const inter = intersection(boundryNode, lastPointOutside, point2);
      mermaid_core.log.warn("abc88 inside", point2, lastPointOutside, inter);
      mermaid_core.log.warn("abc88 intersection", inter);
      ...
      if (!points.some((e) => e.x === inter.x && e.y === inter.y)) {
        ...
        mermaid_core.log.warn("abc88 no intersect", inter, points);
      }
      isInside = true;
    } else {
      mermaid_core.log.warn("abc88 outside", point2, lastPointOutside);
  ...
  mermaid_core.log.warn("abc88 returning points", points);
  return points;
};
const insertEdge = function(elem, e, edge, clusterDb, diagramType, graph) {
  ...
  mermaid_core.log.info("abc88 InsertEdge: ", edge);
  ...
  if (edge.toCluster) {
    mermaid_core.log.info("to cluster abc88", clusterDb[edge.toCluster]);
    points = cutPathAtIntersect(edge.points, clusterDb[edge.toCluster].node);
    pointsHasChanged = true;
  }
  if (edge.fromCluster) {
    mermaid_core.log.info("from cluster abc88", clusterDb[edge.fromCluster]);
  ...
  if (diagramType === "graph" || diagramType === "flowchart") {
    curve = edge.curve || mermaid_core.curveBasis;
  } else {
  ...
  let url = "";
  if (mermaid_core.getConfig().flowchart.arrowMarkerAbsolute || mermaid_core.getConfig().state.arrowMarkerAbsolute) {
    url = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search;
    url = url.replace(/\(/g, "\\(");
    url = url.replace(/\)/g, "\\)");
  }
  mermaid_core.log.info("arrowTypeStart", edge.arrowTypeStart);
  mermaid_core.log.info("arrowTypeEnd", edge.arrowTypeEnd);
