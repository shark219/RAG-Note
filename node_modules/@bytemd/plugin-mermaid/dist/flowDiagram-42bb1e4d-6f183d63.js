...
const addHtmlLabel = require("./add-html-label-d720b2f3.js");
...
const styles4fe3d1fc = require("./styles-4fe3d1fc-ec8c7e30.js");
const selectAll = require("./selectAll-87e3a1ba.js");
require("./array-87dee35f.js");
require("./path-186271b6.js");
require("./index-f9d09cc9-3a691079.js");
require("./edges-65da65dc-d0d46691.js");
require("./svgDraw-6a237a99-9b78f7fb.js");
...
function normal(parent, id, edge, type) {
  ...
  addHtmlLabel.applyStyle(path, edge[type + "Style"]);
  if (edge[type + "Class"]) {
...
function addSVGLabel(root, node) {
  ...
  addHtmlLabel.applyStyle(domNode, node.labelStyle);
  return domNode;
}
...
function addLabel(root, node, location2) {
  ...
  if (node.labelType === "svg") {
    ...
    addHtmlLabel.addHtmlLabel(labelSvg, node);
  } else {
...
var createClusters = function(selection, g) {
  var clusters = g.nodes().filter(function(v) {
    return addHtmlLabel.isSubgraph(g, v);
  });
  var svgClusters = selection.selectAll("g.cluster").data(clusters, function(v) {
    return v;
  });
  addHtmlLabel.applyTransition(svgClusters.exit(), g).style("opacity", 0).remove();
  var enterSelection = svgClusters.enter().append("g").attr("class", "cluster").attr("id", function(v) {
    var node = g.node(v);
    return node.id;
  }).style("opacity", 0).each(function(v) {
    var node = g.node(v);
    var thisGroup = mermaid_core.select(this);
    mermaid_core.select(this).append("rect");
    var labelGroup = thisGroup.append("g").attr("class", "label");
    addLabel(labelGroup, node, node.clusterLabelPos);
  });
  svgClusters = svgClusters.merge(enterSelection);
  svgClusters = addHtmlLabel.applyTransition(svgClusters, g).style("opacity", 1);
  svgClusters.selectAll("rect").each(function(c) {
    var node = g.node(c);
    var domCluster = mermaid_core.select(this);
    addHtmlLabel.applyStyle(domCluster, node.style);
  });
  return svgClusters;
};
...
let createEdgeLabels = function(selection, g) {
  var svgEdgeLabels = selection.selectAll("g.edgeLabel").data(g.edges(), function(e) {
    return addHtmlLabel.edgeToId(e);
  }).classed("update", true);
  svgEdgeLabels.exit().remove();
  svgEdgeLabels.enter().append("g").classed("edgeLabel", true).style("opacity", 0);
  svgEdgeLabels = selection.selectAll("g.edgeLabel");
  svgEdgeLabels.each(function(e) {
    var root = mermaid_core.select(this);
    ...
    if (!layout.has(edge, "width")) {
      edge.width = bbox.width;
    }
    if (!layout.has(edge, "height")) {
  ...
  addHtmlLabel.applyTransition(exitSelection, g).style("opacity", 0).remove();
...
var createEdgePaths = function(selection, g, arrows2) {
  ...
  addHtmlLabel.applyTransition(svgPaths, g).style("opacity", 1);
  svgPaths.each(function(e) {
    var domEdge = mermaid_core.select(this);
    var edge = g.edge(e);
    edge.elem = this;
    if (edge.id) {
      domEdge.attr("id", edge.id);
    }
    addHtmlLabel.applyClass(
      domEdge,
      edge["class"],
  ...
  svgPaths.selectAll("path.path").each(function(e) {
    ...
    var domEdge = mermaid_core.select(this).attr("marker-end", function() {
      return "url(" + makeFragmentRef(location.href, edge.arrowheadId) + ")";
    }).style("fill", "none");
    addHtmlLabel.applyTransition(domEdge, g).attr("d", function(e2) {
      return calcPoints(g, e2);
    });
    addHtmlLabel.applyStyle(domEdge, edge.style);
  });
  svgPaths.selectAll("defs *").remove();
  svgPaths.selectAll("defs").each(function(e) {
    var edge = g.edge(e);
    var arrowhead = arrows2[edge.arrowhead];
    arrowhead(mermaid_core.select(this), edge.arrowheadId, edge, "arrowhead");
  });
  return svgPaths;
};
...
function createLine(edge, points) {
  var line = (isPlainObject.line || svg.line)().x(function(d) {
    return d.x;
  }).y(function(d) {
    return d.y;
  });
...
function exit(svgPaths, g) {
  ...
  addHtmlLabel.applyTransition(svgPathExit, g).style("opacity", 0).remove();
}
var createNodes = function(selection, g, shapes2) {
  ...
  svgNodes.each(function(v) {
    ...
    if (layout.has(node, "width")) {
      bbox.width = node.width;
    }
    if (layout.has(node, "height")) {
    ...
    addHtmlLabel.applyStyle(shapeSvg, node.style);
...
function positionClusters(selection, g) {
  var created = selection.filter(function() {
    return !mermaid_core.select(this).classed("update");
  });
  ...
  addHtmlLabel.applyTransition(selection, g).style("opacity", 1).attr("transform", translate);
  addHtmlLabel.applyTransition(created.selectAll("rect"), g).attr("width", function(v) {
...
function positionEdgeLabels(selection, g) {
  ...
  function translate(e) {
    var edge = g.edge(e);
    return layout.has(edge, "x") ? "translate(" + edge.x + "," + edge.y + ")" : "";
  }
  created.attr("transform", translate);
  addHtmlLabel.applyTransition(selection, g).style("opacity", 1).attr("transform", translate);
}
...
function preProcessGraph(g) {
  g.nodes().forEach(function(v) {
    var node = g.node(v);
    if (!layout.has(node, "label") && !g.children(v).length) {
      node.label = v;
    }
    if (layout.has(node, "paddingX")) {
      layout.defaults(node, {
        paddingLeft: node.paddingX,
        paddingRight: node.paddingX
      });
    }
    if (layout.has(node, "paddingY")) {
      layout.defaults(node, {
        paddingTop: node.paddingY,
        paddingBottom: node.paddingY
      });
    }
    if (layout.has(node, "padding")) {
      layout.defaults(node, {
    ...
    layout.forEach(["paddingLeft", "paddingRight", "paddingTop", "paddingBottom"], function(k) {
...
function postProcessGraph(g) {
  layout.forEach(g.nodes(), function(v) {
    var node = g.node(v);
    if (layout.has(node, "_prevWidth")) {
      node.width = node._prevWidth;
    } else {
      delete node.width;
    }
    if (layout.has(node, "_prevHeight")) {
...
const addVertices = function(vert, g, svgId, root, _doc, diagObj) {
  const svg2 = !root ? mermaid_core.select(`[id="${svgId}"]`) : root.select(`[id="${svgId}"]`);
  ...
  keys.forEach(function(id) {
    ...
    const styles = mermaid_core.getStylesFromArray(vertex.styles);
    ...
    if (mermaid_core.evaluate(mermaid_core.getConfig().flowchart.htmlLabels)) {
      ...
      vertexNode = addHtmlLabel.addHtmlLabel(svg2, node).node();
      vertexNode.parentNode.removeChild(vertexNode);
    } else {
      const svgLabel = doc.createElementNS("http://www.w3.org/2000/svg", "text");
      svgLabel.setAttribute("style", styles.labelStyle.replace("color:", "fill:"));
      const rows = vertexText.split(mermaid_core.common.lineBreakRegex);
    ...
    mermaid_core.log.warn("Adding node", vertex.id, vertex.domId);
...
const addEdges = function(edges, g, diagObj) {
  ...
  if (edges.defaultStyle !== void 0) {
    const defaultStyles = mermaid_core.getStylesFromArray(edges.defaultStyle);
  ...
  edges.forEach(function(edge) {
    ...
    if (edge.style !== void 0) {
      const styles = mermaid_core.getStylesFromArray(edge.style);
      style = styles.style;
      labelStyle = styles.labelStyle;
    } else {
      switch (edge.stroke) {
    ...
    if (edge.interpolate !== void 0) {
      edgeData.curve = mermaid_core.interpolateToCurve(edge.interpolate, mermaid_core.curveLinear);
    } else if (edges.defaultInterpolate !== void 0) {
      edgeData.curve = mermaid_core.interpolateToCurve(edges.defaultInterpolate, mermaid_core.curveLinear);
    } else {
      edgeData.curve = mermaid_core.interpolateToCurve(conf.curve, mermaid_core.curveLinear);
    }
    if (edge.text === void 0) {
      if (edge.style !== void 0) {
...
const getClasses = function(text2, diagObj) {
  mermaid_core.log.info("Extracting classes");
  diagObj.db.clear();
  try {
    diagObj.parse(text2);
...
const draw = function(text2, id, _version, diagObj) {
  mermaid_core.log.info("Drawing flowchart");
  diagObj.db.clear();
  const { securityLevel, flowchart: conf2 } = mermaid_core.getConfig();
  let sandboxElement;
  ...
  try {
    ...
    mermaid_core.log.debug("Parsing failed");
  }
  ...
  mermaid_core.log.warn("Get vertices", vert);
  const edges = diagObj.db.getEdges();
  let i = 0;
  for (i = subGraphs.length - 1; i >= 0; i--) {
    subG = subGraphs[i];
    selectAll.selectAll("cluster").append("text");
  ...
  for (i = 0; i < subGraphs.length; i++) {
    ...
    if (subG.title !== "undefined") {
      ...
      const cluster = mermaid_core.select(clusterEl[0]);
  ...
  mermaid_core.setupGraphViewbox(g, svg2, conf2.diagramPadding, conf2.useMaxWidth);
  const keys = Object.keys(vert);
...
const diagram = {
  parser: addHtmlLabel.parser$1,
  db: addHtmlLabel.flowDb,
  renderer: styles4fe3d1fc.flowRendererV2,
  styles: styles4fe3d1fc.flowStyles,
  init: (cnf) => {
    if (!cnf.flowchart) {
      cnf.flowchart = {};
    }
    ...
    addHtmlLabel.flowDb.clear();
    addHtmlLabel.flowDb.setGen("gen-1");
  }
};
